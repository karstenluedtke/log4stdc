               ...
               const char *func, const char *fmt, ...)
{
    ...
    if (l4sc_logger_enabled(logger, level)) {
        ...
        if (file)
            estimate += strlen(file);
        if (func)
            estimate += strlen(func);

        do {
            va_start(ap, fmt);
            rc = l4sc_vlog(logger, level, estimate, 0, file, line, func, fmt,
                           ap);
            va_end(ap);
            if (rc > estimate) {
                estimate = rc + 80;
        ...
        va_start(ap, fmt);
        rc = l4sc_vlog(logger, level, estimate, 1, file, line, func, fmt, ap);
        va_end(ap);
    }
