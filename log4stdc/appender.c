...
static int
set_appender_layout(l4sc_appender_ptr_t appender, l4sc_layout_ptr_t layout);
...
init_appender(void *buf, size_t bufsize, bfc_mempool_t pool)
{
    BFC_INIT_PROLOGUE(l4sc_appender_class_ptr_t, l4sc_appender_ptr_t, appender,
                      buf, bufsize, pool, &l4sc_sysout_appender_class);

    appender->name = "sysout appender";
    appender->layout.vptr = &l4sc_patternlayout_class;
    VOID_METHCALL(l4sc_layout_class_ptr_t, &appender->layout, init,
...
append_to_output(l4sc_appender_ptr_t appender, l4sc_logmessage_cptr_t msg)
{
    if (msg && (msg->msglen > 0)) {
        ...
        const char *tag = msg->logger ? msg->logger->name : appender->name;
...
l4sc_get_extra_class(const char *name, size_t namlen)
{
    ...
    for (i = 0; i < l4sc_num_extra_classes; i++) {
        if ((cls = l4sc_extra_classes[i]) && cls->name) {
            ...
            } else if ((namlen > clen) && (name[namlen - clen - 1] == '.') &&
                       (strncasecmp(cls->name, name + namlen - clen, namlen) ==
                        0)) {
                return (cls);
            }
        }
...
l4sc_get_appender(const char *name, int nlen, const char *kind, int klen)
{
    ...
    if (kind && (klen >= 5)) {
        ...
        BFC_LIST_FOREACH(appender, &l4sc_appenders, next)
        {
            ...
            if (cl && cl->name && (strncasecmp(cl->name, kind, klen) == 0) &&
                (cl->name[klen] == '\0')) {
                clazz = cl;
            }
        }
        ...
        LOGINFO(("%s: appender %.*s not found, no class", __FUNCTION__, nlen,
                 name));
        return (NULL);
    }
    ...
    BFC_LIST_FOREACH(appender, &l4sc_appenders, next)
    {
        if ((strncasecmp(appender->name, name, nlen) == 0) &&
            ...
            if (cl == clazz) {
                ...
                LOGINFO(("%s: appender %s class %s != %s", __FUNCTION__,
                         appender->name, cl ? cl->name : "(null)",
                         clazz->name));
                appender->name = obsolete_appender_name;
                VOID_METHCALL(l4sc_appender_class_ptr_t, appender, close,
                              (appender));
                break;
            }
        }
    ...
    LOGINFO(("%s: appender %.*s not found, creating %.*s ...", __FUNCTION__,
             nlen, name, klen, kind));
    RETVAR_CMETHCALL(size, l4sc_appender_class_ptr_t, clazz, clonesize, (NULL),
                     get_appender_size(NULL));
    appender = mempool_alloc(pool, size);
    if (appender != NULL) {
        RETVAR_CMETHCALL(rc, l4sc_appender_class_ptr_t, clazz, init,
                         (appender, size, pool), -1);
        if (rc >= 0) {
            appender->parent_pool = pool;
            VOID_METHCALL(l4sc_appender_class_ptr_t, appender, initrefc,
                          (appender, 1));
...
l4sc_close_appenders(void)
{
    ...
    BFC_LIST_FOREACH(appender, &l4sc_appenders, next)
    {
        VOID_METHCALL(l4sc_appender_class_ptr_t, appender, close, (appender));
    }
}
