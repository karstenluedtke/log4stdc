...
static int
init_mutex(void *, size_t, bfc_mempool_t);
static int
clone_mutex(bfc_mutex_ptr_t, void *, size_t, bfc_mempool_t);
static void destroy_mutex(bfc_mutex_ptr_t);
static size_t mutex_size(bfc_mutex_ptr_t);
static void
dump_mutex(bfc_mutex_ptr_t, int, struct l4sc_logger *);

static bfc_mutex_ptr_t
mutex_lock(bfc_mutex_ptr_t, const char *, int, const char *);
static void
mutex_unlock(bfc_mutex_ptr_t, const char *, int, const char *);
...
{
    BFC_INIT_PROLOGUE(const struct bfc_mutex_class *, struct posix_mutex *,
                      object, buf, bufsize, pool, &l4sc_posix_mutex_class);
    if (object) {
...
static int
clone_mutex(bfc_mutex_ptr_t obj, void *buf, size_t bufsize, bfc_mempool_t pool)
{
...
dump_mutex(bfc_mutex_ptr_t obj, int depth, struct l4sc_logger *log)
{
    ...
    if (obj && obj->name && BFC_CLASS(obj)) {
        ...
        l4sc_logprintf(log, INFO_LEVEL, __FILE__, __LINE__, thisfunction,
                       ...
                       obj->refc);
        l4sc_logprintf(log, INFO_LEVEL, __FILE__, __LINE__, thisfunction,
                       " last access from %s in %s:%d", obj->func, obj->file,
                       obj->line);
    }
}
