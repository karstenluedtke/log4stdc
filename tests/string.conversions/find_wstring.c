...
test(const char *s1, const wchar_t *s2, size_t pos1, size_t expected)
{
	...
	pos = bfc_string_find_bfstr(&s, &ws, pos1);
	assert (pos == expected);
...
#define S2(x) L##x
	...
	test(S1("\303\244bcdefghijklmn\303\266pqrst\303\274"), S2("bcd"), 0, 2);
	do {
	    wchar_t s2[] = { 0xF6, 'p', 'q' , 0 };
	    test(S1("\303\244bcdefghijklmn\303\266pqrst\303\274"), s2, 0, 15);
	} while (0 /* just once */);
	do {
	    wchar_t s2[] = { 'm', 'n', 0xF6, 'p', 'q' , 0 };
	    test(S1("\303\244bcdefghijklmn\303\266pqrst\303\274"), s2, 0, 13);
	} while (0 /* just once */);
