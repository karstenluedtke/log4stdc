...
void test0()
{
    ...
    test(S(L"cdafr"), L"romds", 0, 5, S::npos);
    ...
    test(S(L"mhqgd"), L"pqscrjthli", 0, 10, S::npos);
    ...
    test(S(L"kmspj"), L"lasiq", 1, 4, S::npos);
    test(S(L"tjboh"), L"kfqmr", 1, 5, S::npos);
    test(S(L"ilbcj"), L"klnitfaobg", 1, 0, S::npos);
    test(S(L"jkngf"), L"gjhmdlqikp", 1, 1, S::npos);
    test(S(L"gfcql"), L"skbgtahqej", 1, 5, 0);
    test(S(L"dqtlg"), L"bjsdgtlpkf", 1, 9, 0);
    test(S(L"bthpg"), L"bjgfmnlkio", 1, 10, 0);
    ...
    test(S(L"smaqd"), L"dpifl", 2, 2, S::npos);
    test(S(L"hjeni"), L"oapht", 2, 4, 0);
    test(S(L"ocmfj"), L"cifts", 2, 5, 1);
    ...
    test(S(L"thlnq"), L"ndkjeisgcl", 4, 5, 3);
...
void test1()
{
    test(S(L"gbhqo"), L"skqne", 5, 4, 3);
    test(S(L"ktdor"), L"kipnf", 5, 5, 0);
    test(S(L"ldprn"), L"hmrnqdgifl", 5, 0, S::npos);
    test(S(L"egmjk"), L"fsmjcdairn", 5, 1, S::npos);
    test(S(L"armql"), L"pcdgltbrfj", 5, 5, 4);
    test(S(L"cdhjo"), L"aekfctpirg", 5, 9, 0);
    test(S(L"jcons"), L"ledihrsgpf", 5, 10, 4);
    test(S(L"cbrkp"), L"mqcklahsbtirgopefndj", 5, 0, S::npos);
    test(S(L"fhgna"), L"kmlthaoqgecrnpdbjfis", 5, 1, S::npos);
    test(S(L"ejfcd"), L"sfhbamcdptojlkrenqgi", 5, 10, 4);
    test(S(L"kqjhe"), L"pbniofmcedrkhlstgaqj", 5, 19, 4);
    test(S(L"pbdjl"), L"mongjratcskbhqiepfdl", 5, 20, 4);
    ...
    test(S(L"klhde"), L"ohcmb", 6, 4, 2);
    test(S(L"bhlki"), L"heatr", 6, 5, 1);
    test(S(L"lqmoh"), L"pmblckedfn", 6, 0, S::npos);
    test(S(L"mtqin"), L"aceqmsrbik", 6, 1, S::npos);
    test(S(L"dpqbr"), L"lmbtdehjrn", 6, 5, 3);
    test(S(L"kdhmo"), L"teqmcrlgib", 6, 9, 3);
    test(S(L"jblqp"), L"njolbmspac", 6, 10, 4);
    test(S(L"qmjgl"), L"pofnhidklamecrbqjgst", 6, 0, S::npos);
    test(S(L"rothp"), L"jbhckmtgrqnosafedpli", 6, 1, S::npos);
    test(S(L"ghknq"), L"dobntpmqklicsahgjerf", 6, 10, 4);
    test(S(L"eopfi"), L"tpdshainjkbfoemlrgcq", 6, 19, 4);
    test(S(L"dsnmg"), L"oldpfgeakrnitscbjmqh", 6, 20, 4);
    test(S(L"jnkrfhotgl"), L"", 0, 0, S::npos);
    test(S(L"dltjfngbko"), L"rqegt", 0, 0, S::npos);
    test(S(L"bmjlpkiqde"), L"dashm", 0, 1, S::npos);
    test(S(L"skrflobnqm"), L"jqirk", 0, 2, S::npos);
    test(S(L"jkpldtshrm"), L"rckeg", 0, 4, S::npos);
    test(S(L"ghasdbnjqo"), L"jscie", 0, 5, S::npos);
    test(S(L"igrkhpbqjt"), L"efsphndliq", 0, 0, S::npos);
    test(S(L"ikthdgcamf"), L"gdicosleja", 0, 1, S::npos);
    test(S(L"pcofgeniam"), L"qcpjibosfl", 0, 5, 0);
    test(S(L"rlfjgesqhc"), L"lrhmefnjcq", 0, 9, 0);
    test(S(L"itphbqsker"), L"dtablcrseo", 0, 10, S::npos);
    test(S(L"skjafcirqm"), L"apckjsftedbhgomrnilq", 0, 0, S::npos);
    test(S(L"tcqomarsfd"), L"pcbrgflehjtiadnsokqm", 0, 1, S::npos);
    test(S(L"rocfeldqpk"), L"nsiadegjklhobrmtqcpf", 0, 10, S::npos);
    ...
    test(S(L"fjiknedcpq"), L"anqhk", 1, 4, S::npos);
    ...
    test(S(L"omchkfrjea"), L"gbkqdoeftl", 1, 9, 0);
    ...
    test(S(L"astedncjhk"), L"qcloh", 5, 2, S::npos);
    test(S(L"fhlqgcajbr"), L"thlmp", 5, 4, 2);
    test(S(L"epfhocmdng"), L"qidmo", 5, 5, 4);
    test(S(L"apcnsibger"), L"lnegpsjqrd", 5, 0, S::npos);
    test(S(L"aqkocrbign"), L"rjqdablmfs", 5, 1, 5);
    test(S(L"ijsmdtqgce"), L"enkgpbsjaq", 5, 5, S::npos);
    test(S(L"clobgsrken"), L"kdsgoaijfh", 5, 9, 5);
    test(S(L"jbhcfposld"), L"trfqgmckbe", 5, 10, 4);
    ...
    test(S(L"ngcrieqajf"), L"iqfkm", 9, 2, 6);
    test(S(L"qosmilgnjb"), L"tqjsr", 9, 4, 8);
    test(S(L"ikabsjtdfl"), L"jplqg", 9, 5, 9);
    test(S(L"ersmicafdh"), L"oilnrbcgtj", 9, 0, S::npos);
    test(S(L"fdnplotmgh"), L"morkglpesn", 9, 1, 7);
    test(S(L"fdbicojerm"), L"dmicerngat", 9, 5, 9);
    test(S(L"mbtafndjcq"), L"radgeskbtc", 9, 9, 6);
    test(S(L"mlenkpfdtc"), L"ljikprsmqo", 9, 10, 5);
    test(S(L"ahlcifdqgs"), L"trqihkcgsjamfdbolnpe", 9, 0, S::npos);
    test(S(L"bgjemaltks"), L"lqmthbsrekajgnofcipd", 9, 1, 6);
    test(S(L"pdhslbqrfc"), L"jtalmedribkgqsopcnfh", 9, 10, 7);
    ...
    test(S(L"pqfhsgilen"), L"qnpor", 10, 1, 1);
    test(S(L"gqtjsbdckh"), L"otdma", 10, 2, 2);
    test(S(L"cfkqpjlegi"), L"efhjg", 10, 4, 7);
    test(S(L"beanrfodgj"), L"odpte", 10, 5, 7);
    test(S(L"adtkqpbjfi"), L"bctdgfmolr", 10, 0, S::npos);
    test(S(L"iomkfthagj"), L"oaklidrbqg", 10, 1, 1);
}

template <class S>
void test2()
{
    test(S(L"sdpcilonqj"), L"dnjfsagktr", 10, 5, 9);
    test(S(L"gtfbdkqeml"), L"nejaktmiqg", 10, 9, 8);
    test(S(L"bmeqgcdorj"), L"pjqonlebsf", 10, 10, 9);
    test(S(L"etqlcanmob"), L"dshmnbtolcjepgaikfqr", 10, 0, S::npos);
    test(S(L"roqmkbdtia"), L"iogfhpabtjkqlrnemcds", 10, 1, 8);
    test(S(L"kadsithljf"), L"ngridfabjsecpqltkmoh", 10, 10, 9);
    test(S(L"sgtkpbfdmh"), L"athmknplcgofrqejsdib", 10, 19, 9);
    test(S(L"qgmetnabkl"), L"ldobhmqcafnjtkeisgrp", 10, 20, 9);
    test(S(L"cqjohampgd"), L"", 11, 0, S::npos);
    test(S(L"hobitmpsan"), L"aocjb", 11, 0, S::npos);
    test(S(L"tjehkpsalm"), L"jbrnk", 11, 1, 1);
    test(S(L"ngfbojitcl"), L"tqedg", 11, 2, 7);
    test(S(L"rcfkdbhgjo"), L"nqskp", 11, 4, 3);
    test(S(L"qghptonrea"), L"eaqkl", 11, 5, 9);
    test(S(L"hnprfgqjdl"), L"reaoicljqm", 11, 0, S::npos);
    test(S(L"hlmgabenti"), L"lsftgajqpm", 11, 1, 1);
    test(S(L"ofcjanmrbs"), L"rlpfogmits", 11, 5, 7);
    test(S(L"jqedtkornm"), L"shkncmiaqj", 11, 9, 9);
    test(S(L"rfedlasjmg"), L"fpnatrhqgs", 11, 10, 9);
    test(S(L"talpqjsgkm"), L"sjclemqhnpdbgikarfot", 11, 0, S::npos);
    test(S(L"lrkcbtqpie"), L"otcmedjikgsfnqbrhpla", 11, 1, S::npos);
    test(S(L"cipogdskjf"), L"bonsaefdqiprkhlgtjcm", 11, 10, 9);
    test(S(L"nqedcojahi"), L"egpscmahijlfnkrodqtb", 11, 19, 9);
    test(S(L"hefnrkmctj"), L"kmqbfepjthgilscrndoa", 11, 20, 9);
    test(S(L"atqirnmekfjolhpdsgcb"), L"", 0, 0, S::npos);
    test(S(L"echfkmlpribjnqsaogtd"), L"prboq", 0, 0, S::npos);
    test(S(L"qnhiftdgcleajbpkrosm"), L"fjcqh", 0, 1, S::npos);
    test(S(L"chamfknorbedjitgslpq"), L"fmosa", 0, 2, S::npos);
    test(S(L"njhqpibfmtlkaecdrgso"), L"qdbok", 0, 4, S::npos);
    test(S(L"ebnghfsqkprmdcljoiat"), L"amslg", 0, 5, S::npos);
    test(S(L"letjomsgihfrpqbkancd"), L"smpltjneqb", 0, 0, S::npos);
    test(S(L"nblgoipcrqeaktshjdmf"), L"flitskrnge", 0, 1, S::npos);
    test(S(L"cehkbngtjoiflqapsmrd"), L"pgqihmlbef", 0, 5, S::npos);
    test(S(L"mignapfoklbhcqjetdrs"), L"cfpdqjtgsb", 0, 9, S::npos);
    test(S(L"ceatbhlsqjgpnokfrmdi"), L"htpsiaflom", 0, 10, S::npos);
    test(S(L"ocihkjgrdelpfnmastqb"), L"kpjfiaceghsrdtlbnomq", 0, 0, S::npos);
    test(S(L"noelgschdtbrjfmiqkap"), L"qhtbomidljgafneksprc", 0, 1, S::npos);
    test(S(L"dkclqfombepritjnghas"), L"nhtjobkcefldimpsaqgr", 0, 10, S::npos);
    ...
    test(S(L"reagphsqflbitdcjmkno"), L"smfte", 1, 1, S::npos);
    test(S(L"jitlfrqemsdhkopncabg"), L"ciboh", 1, 2, 1);
    test(S(L"mhtaepscdnrjqgbkifol"), L"haois", 1, 4, 1);
    test(S(L"tocesrfmnglpbjihqadk"), L"abfki", 1, 5, S::npos);
    test(S(L"lpfmctjrhdagneskbqoi"), L"frdkocntmq", 1, 0, S::npos);
    test(S(L"lsmqaepkdhncirbtjfgo"), L"oasbpedlnr", 1, 1, S::npos);
    test(S(L"epoiqmtldrabnkjhcfsg"), L"kltqmhgand", 1, 5, S::npos);
    ...
    test(S(L"atjgfsdlpobmeiqhncrk"), L"ljqprsmigtfoneadckbh", 1, 1, S::npos);
    ...
    test(S(L"mkpnblfdsahrcqijteog"), L"cahif", 10, 1, S::npos);
    test(S(L"gckarqnelodfjhmbptis"), L"kehis", 10, 2, 7);
    test(S(L"gqpskidtbclomahnrjfe"), L"kdlmh", 10, 4, 10);
    test(S(L"pkldjsqrfgitbhmaecno"), L"paeql", 10, 5, 6);
    test(S(L"aftsijrbeklnmcdqhgop"), L"aghoqiefnb", 10, 0, S::npos);
    test(S(L"mtlgdrhafjkbiepqnsoc"), L"jrbqaikpdo", 10, 1, 9);
    test(S(L"pqgirnaefthokdmbsclj"), L"smjonaeqcl", 10, 5, 5);
    test(S(L"kpdbgjmtherlsfcqoina"), L"eqbdrkcfah", 10, 9, 10);
    test(S(L"jrlbothiknqmdgcfasep"), L"kapmsienhf", 10, 10, 9);
    test(S(L"mjogldqferckabinptsh"), L"jpqotrlenfcsbhkaimdg", 10, 0, S::npos);
    test(S(L"apoklnefbhmgqcdrisjt"), L"jlbmhnfgtcqprikeados", 10, 1, S::npos);
    ...
    test(S(L"rloadknfbqtgmhcsipje"), L"iclat", 19, 1, 16);
    test(S(L"mgjhkolrnadqbpetcifs"), L"rkhnf", 19, 2, 7);
    test(S(L"cmlfakiojdrgtbsphqen"), L"clshq", 19, 4, 16);
    test(S(L"kghbfipeomsntdalrqjc"), L"dtcoj", 19, 5, 19);
    test(S(L"eldiqckrnmtasbghjfpo"), L"rqosnjmfth", 19, 0, S::npos);
    test(S(L"abqjcfedgotihlnspkrm"), L"siatdfqglh", 19, 1, 15);
    test(S(L"qfbadrtjsimkolcenhpg"), L"mrlshtpgjq", 19, 5, 17);
    test(S(L"abseghclkjqifmtodrnp"), L"adlcskgqjt", 19, 9, 16);
    test(S(L"ibmsnlrjefhtdokacqpg"), L"drshcjknaf", 19, 10, 16);
    test(S(L"mrkfciqjebaponsthldg"), L"etsaqroinghpkjdlfcbm", 19, 0, S::npos);
    test(S(L"mjkticdeoqshpalrfbgn"), L"sgepdnkqliambtrocfhj", 19, 1, 10);
    ...
    test(S(L"grkpahljcftesdmonqib"), L"odife", 20, 1, 15);
    test(S(L"jimlgbhfqkteospardcn"), L"okaqd", 20, 2, 12);
    test(S(L"gftenihpmslrjkqadcob"), L"lcdbi", 20, 4, 19);
    test(S(L"bmhldogtckrfsanijepq"), L"fsqbj", 20, 5, 19);
    test(S(L"nfqkrpjdesabgtlcmoih"), L"bigdomnplq", 20, 0, S::npos);
    test(S(L"focalnrpiqmdkstehbjg"), L"apiblotgcd", 20, 1, 3);
    test(S(L"rhqdspkmebiflcotnjga"), L"acfhdenops", 20, 5, 19);
    test(S(L"rahdtmsckfboqlpniegj"), L"jopdeamcrk", 20, 9, 19);
    test(S(L"fbkeiopclstmdqranjhg"), L"trqncbkgmh", 20, 10, 19);
...
void test3()
{
    test(S(L"pboqganrhedjmltsicfk"), L"gbkhdnpoietfcmrslajq", 20, 1, 4);
    test(S(L"klchabsimetjnqgorfpd"), L"rtfnmbsglkjaichoqedp", 20, 10, 17);
    test(S(L"sirfgmjqhctndbklaepo"), L"ohkmdpfqbsacrtjnlgei", 20, 19, 19);
    test(S(L"rlbdsiceaonqjtfpghkm"), L"dlbrteoisgphmkncajfq", 20, 20, 19);
    test(S(L"ecgdanriptblhjfqskom"), L"", 21, 0, S::npos);
    test(S(L"fdmiarlpgcskbhoteqjn"), L"sjrlo", 21, 0, S::npos);
    test(S(L"rlbstjqopignecmfadkh"), L"qjpor", 21, 1, 6);
    test(S(L"grjpqmbshektdolcafni"), L"odhfn", 21, 2, 13);
    test(S(L"sakfcohtqnibprjmlged"), L"qtfin", 21, 4, 10);
    test(S(L"mjtdglasihqpocebrfkn"), L"hpqfo", 21, 5, 17);
    test(S(L"okaplfrntghqbmeicsdj"), L"fabmertkos", 21, 0, S::npos);
    test(S(L"sahngemrtcjidqbklfpo"), L"brqtgkmaej", 21, 1, 14);
    test(S(L"dlmsipcnekhbgoaftqjr"), L"nfrdeihsgl", 21, 5, 19);
    test(S(L"ahegrmqnoiklpfsdbcjt"), L"hlfrosekpi", 21, 9, 14);
    test(S(L"hdsjbnmlegtkqripacof"), L"atgbkrjdsm", 21, 10, 16);
    test(S(L"pcnedrfjihqbalkgtoms"), L"blnrptjgqmaifsdkhoec", 21, 0, S::npos);
    test(S(L"qjidealmtpskrbfhocng"), L"ctpmdahebfqjgknloris", 21, 1, 17);
    test(S(L"qeindtagmokpfhsclrbj"), L"apnkeqthrmlbfodiscgj", 21, 10, 17);
    test(S(L"kpfegbjhsrnodltqciam"), L"jdgictpframeoqlsbknh", 21, 19, 19);
    test(S(L"hnbrcplsjfgiktoedmaq"), L"qprlsfojamgndekthibc", 21, 20, 19);
}
