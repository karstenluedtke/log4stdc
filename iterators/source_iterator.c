...
init_iterator(void *buf, size_t bufsize, struct mempool *pool)
{
	...
	it->vptr = &bfc_source_forward_iterator_class;
	return (BFC_SUCCESS);
}

int
bfc_init_source_iterator(void *buf, size_t bufsize, long sourceval, size_t pos)
{
	int rc;
	l4sc_logger_ptr_t logger = l4sc_get_logger("barefootc.string", 16);
	L4SC_TRACE(logger, "%s(%p, %ld, %ld, %ld)",
		__FUNCTION__, buf, (long) bufsize, sourceval, (long) pos);

	if ((rc = init_iterator(buf, bufsize, NULL)) == BFC_SUCCESS) {
		bfc_iterptr_t it = (bfc_iterptr_t) buf;
		it->obj = (bfc_objptr_t) (intptr_t) sourceval;
		it->pos = pos;
	}
...
int
bfc_init_source_reverse_iterator(void *buf, size_t bufsize,
				long sourceval, size_t pos)
{
	...
	rc = bfc_init_source_iterator(buf, bufsize, sourceval, pos);
	it->vptr = &bfc_source_reverse_iterator_class;
	if (pos == BFC_NPOS) {
...
{
	if (it && BFC_CLASS(it)) {
		L4SC_DEBUG(log, "%s @%p, sourcing %ld",
			BFC_CLASS(it)->name, it, (long)(intptr_t) it->obj);
	}
}
