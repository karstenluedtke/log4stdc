...
void test0()
{
    ...
    test(S(L"pnracgfkjdiholtbqsem"), S(L"tboimldpjh"), 0, S::npos);
    ...
    test(S(L"rbtaqjhgkneisldpmfoc"), S(L"gtfblmqinc"), 1, 0);
    test(S(L"gpifsqlrdkbonjtmheca"), S(L"mkqpbtdalgniorhfescj"), 1, S::npos);
    test(S(L"hdpkobnsalmcfijregtq"), S(L""), 10, 10);
    test(S(L"jtlshdgqaiprkbcoenfm"), S(L"pblas"), 10, 9);
    test(S(L"fkdrbqltsgmcoiphneaj"), S(L"arosdhcfme"), 10, 9);
    ...
    test(S(L"mrtaefilpdsgocnhqbjk"), S(L"lgokshjtpb"), 19, 16);
    test(S(L"kojatdhlcmigpbfrqnes"), S(L"bqjhtkfepimcnsgrlado"), 19, S::npos);
    test(S(L"eaintpchlqsbdgrkjofm"), S(L""), 20, 19);
    test(S(L"gjnhidfsepkrtaqbmclo"), S(L"nocfa"), 20, 18);
    test(S(L"spocfaktqdbiejlhngmr"), S(L"bgtajmiedc"), 20, 19);
    test(S(L"rphmlekgfscndtaobiqj"), S(L"lsckfnqgdahejiopbtmr"), 20, S::npos);
    test(S(L"liatsqdoegkmfcnbhrpj"), S(L""), 21, 19);
    test(S(L"binjagtfldkrspcomqeh"), S(L"gfsrt"), 21, 19);
    test(S(L"latkmisecnorjbfhqpdg"), S(L"pfsocbhjtm"), 21, 19);
...
void test1()
{
    ...
    test(S(L"nhmko"), S(L""), 4);
    test(S(L"lahfb"), S(L"irkhs"), 4);
    test(S(L"gmfhd"), S(L"kantesmpgj"), 4);
    test(S(L"odaft"), S(L"oknlrstdpiqmjbaghcfe"), S::npos);
    test(S(L"eolhfgpjqk"), S(L""), 9);
    test(S(L"nbatdlmekr"), S(L"bnrpe"), 8);
    test(S(L"jdmciepkaq"), S(L"jtdaefblso"), 9);
    test(S(L"hkbgspoflt"), S(L"oselktgbcapndfjihrmq"), S::npos);
    test(S(L"gprdcokbnjhlsfmtieqa"), S(L""), 19);
    test(S(L"qjghlnftcaismkropdeb"), S(L"bjaht"), 18);
    test(S(L"pnalfrdtkqcmojiesbhg"), S(L"hjlcmgpket"), 17);
