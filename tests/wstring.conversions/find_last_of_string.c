...
test(const wchar_t *s1, const char *s2, size_t pos1, size_t expected)
{
	...
	pos = bfc_string_find_last_of_bfstr(&s, &pat, pos1);
	assert (pos == expected);
...
#define S2(x) x
	...
	do {
	    ...
	    test(s1, S2("\303\266pq"), BFC_NPOS, 6);
	    test(s1, S2("\303\266rq"), BFC_NPOS, 6);
	    test(s1, S2("\303\266pq"), 3, 3);
	    test(s1, S2("\303\266pq"), 2, 2);
