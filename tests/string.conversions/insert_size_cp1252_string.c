...
static int
test(const char *s1, size_t pos1, const char *s2, const char *expected)
{
	int rc;
	bfc_string_t s, ws;
	size_t origlen, wslen, newlen;

	L4SC_TRACE(logger, "%s(\"%s\", pos %ld, \"%s\", expected \"%s\")",
		__FUNCTION__, s1, (long) pos1, s2, expected);
	...
	rc = bfc_init_shared_cp1252_string_c_str(&ws, sizeof(ws), s2);
	assert(rc >= 0);
	...
	L4SC_TRACE(logger,
		"%s(\"%s\", pos %ld, \"%s\", expected \"%s\"): rc %d",
		"PASS", s1, (long) pos1, s2, expected, rc);
...
#define SE(x) x
	...
	test(S1("bcdefghijklmnopqrst"), 0, S2("\344"),
	     SE("\303\244bcdefghijklmnopqrst"));

	test(S1("abcdefghijklmnpqrst"), 14, S2("\366"),
	     SE("abcdefghijklmn\303\266pqrst"));

	test(S1("abcdefghijklmnopqrst"), 20, S2("\374"),
	     SE("abcdefghijklmnopqrst\303\274"));

	test(S1("abcdeklmnopqrst"), 5, S2("\203ghij"),
	     SE("abcde\306\222ghijklmnopqrst"));
