...
static int
init_property_configurator(void *, size_t, bfc_mempool_t);
static size_t
...
static const char *
trim_front(const char *buf, const char *limit)
{
    const char *cp = buf;

    while ((cp < limit) && (((unsigned char *)cp)[0] <= ' '))
        ++cp;
    return (cp);
}

int
trimmed_len(const char *base, const char *pos)
{
    const char *cp = pos;

    while ((cp > base) && (((unsigned char *)cp)[-1] <= ' '))
        --cp;
    return ((int)(cp - base));
}
...
config_from_property_line(const char *buf, const char *limit, int loop)
{
    ...
    if (strncasecmp(nodetype, "logger.", 7) == 0) {
        ...
        if (logger) {
            ...
            l4sc_set_object_option((l4sc_objptr_t)logger, "level", 5, v,
                                   vallen);
            while (cp && (cp < ep)) {
                v = trim_front(++cp, ep);
                cp = memchr(cp, ',', ep - cp);
                vallen = trimmed_len(v, cp ? cp : ep);
                if ((appender = l4sc_get_appender(v, vallen, NULL, 0)) !=
                    NULL) {
                    l4sc_set_logger_appender(logger, appender);
                } else if (loop == 0) {
            ...
            LOGERROR(("%s: no logger %.*s", thisfunction, nodelen, nodename));
        }
        return (0);
    } else if (strncasecmp(nodetype, "additivity.", 11) == 0) {
        logger = l4sc_get_logger(nodename, nodelen);
        if (logger) {
            l4sc_set_object_option((l4sc_objptr_t)logger, "additivity", 10, v,
                                   vallen);
        }
        return (1);
    } else if (strncasecmp(nodetype, "appender.", 9) == 0) {
        if ((cp = memchr(cp, '.', ep - cp)) == NULL) {
            appender = l4sc_get_appender(nodename, nodelen, v, vallen);
            if (appender == NULL) {
                LOGERROR(("%s: could not create appender"
                          " %.*s, type %.*s",
                          thisfunction, nodelen, nodename, vallen, v));
            }
            return (1);
        } else {
            nodelen = trimmed_len(nodename, cp);
            if ((appender = l4sc_get_appender(nodename, nodelen, NULL, 0)) ==
                NULL) {
                return (0);
            }
            ...
            if (strncasecmp(optname, "layout", 6) == 0) {
                ...
                if ((cp = memchr(cp, '.', ep - cp)) != NULL) {
                    optname = trim_front(++cp, ep);
                    optlen = trimmed_len(optname, ep);
                    l4sc_set_object_option((l4sc_objptr_t)layout, optname,
                                           optlen, v, vallen);
                }
                return (1);
            }
            l4sc_set_object_option((l4sc_objptr_t)appender, optname, optlen, v,
                                   vallen);
            return (1);
        }
    }
