...
static int
test(const wchar_t *s1, const char *s2, const wchar_t *expected)
{
	int rc;
	bfc_string_t s, rs;
	...
	L4SC_TRACE(logger, "%s(L\"%s\", \"%s\", expected L\"%s\")",
		__FUNCTION__, u8buf, s2, exp8buf);
	...
	L4SC_TRACE(logger, "%s(L\"%s\", \"%s\", expected L\"%s\"): rc %d",
			"PASS", u8buf, s2, exp8buf, rc);
...
#define SE(x) L##x
	...
	do {
		...
		test(S1(""), s2, se);
		test(S1("12345"), s2, se);
		test(S1("1234527890123452789012345278901234527890"), s2, se);
	} while(0);
