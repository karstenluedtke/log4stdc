/**
 * @file bfc_string_find_first_not_of_bfstr.c
 */
...
/**
 * @brief    bfc_string_find_first_not_of_bfstr
 */
size_t
bfc_string_find_first_not_of_bfstr(bfc_cstrptr_t s,bfc_cstrptr_t str,
					size_t pos)
{
	if (STRING_TRAITS(s) == STRING_TRAITS(str)) {
		...
		RETURN_METHCALL(bfc_string_classptr_t, s,
			...
			bfc_string_find_first_not_of_buffer(s, pattern,
								pos, patlen));
	} else {
		...
		return bfc_string_find_first_not_of_range(s, &pattern,
								&pattend, pos);
	}
}
