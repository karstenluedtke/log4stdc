...
void test0()
{
    ...
    test(S(L"pnracgfkjdiholtbqsem"), L"tboimldpjh", 0, S::npos);
    ...
    test(S(L"rbtaqjhgkneisldpmfoc"), L"gtfblmqinc", 1, 0);
    test(S(L"gpifsqlrdkbonjtmheca"), L"mkqpbtdalgniorhfescj", 1, S::npos);
    test(S(L"hdpkobnsalmcfijregtq"), L"", 10, 10);
    test(S(L"jtlshdgqaiprkbcoenfm"), L"pblas", 10, 9);
    test(S(L"fkdrbqltsgmcoiphneaj"), L"arosdhcfme", 10, 9);
    ...
    test(S(L"mrtaefilpdsgocnhqbjk"), L"lgokshjtpb", 19, 16);
    test(S(L"kojatdhlcmigpbfrqnes"), L"bqjhtkfepimcnsgrlado", 19, S::npos);
    test(S(L"eaintpchlqsbdgrkjofm"), L"", 20, 19);
    test(S(L"gjnhidfsepkrtaqbmclo"), L"nocfa", 20, 18);
    test(S(L"spocfaktqdbiejlhngmr"), L"bgtajmiedc", 20, 19);
    test(S(L"rphmlekgfscndtaobiqj"), L"lsckfnqgdahejiopbtmr", 20, S::npos);
    test(S(L"liatsqdoegkmfcnbhrpj"), L"", 21, 19);
    test(S(L"binjagtfldkrspcomqeh"), L"gfsrt", 21, 19);
    test(S(L"latkmisecnorjbfhqpdg"), L"pfsocbhjtm", 21, 19);
...
void test1()
{
    ...
    test(S(L"nhmko"), L"", 4);
    test(S(L"lahfb"), L"irkhs", 4);
    test(S(L"gmfhd"), L"kantesmpgj", 4);
    test(S(L"odaft"), L"oknlrstdpiqmjbaghcfe", S::npos);
    test(S(L"eolhfgpjqk"), L"", 9);
    test(S(L"nbatdlmekr"), L"bnrpe", 8);
    test(S(L"jdmciepkaq"), L"jtdaefblso", 9);
    test(S(L"hkbgspoflt"), L"oselktgbcapndfjihrmq", S::npos);
    test(S(L"gprdcokbnjhlsfmtieqa"), L"", 19);
    test(S(L"qjghlnftcaismkropdeb"), L"bjaht", 18);
    test(S(L"pnalfrdtkqcmojiesbhg"), L"hjlcmgpket", 17);
