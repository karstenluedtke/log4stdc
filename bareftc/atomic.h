...
#define bfc_read_atomic_counter(ctr) atomic_load(&(ctr))
...
#define bfc_read_atomic_counter(ctr) __sync_add_and_fetch(&(ctr), 0)

#elif defined(__APPLE__) // deprecated by apple
#include <inttypes.h>
...
#define bfc_read_atomic_counter(ctr) OSAtomicAdd32Barrier(0, &(ctr))
...
#define bfc_read_atomic_counter(ctr) InterlockedExchangeAdd(&(ctr), 0)
...
#define bfc_incr_atomic_counter(ctr) bfc_incr_i386_atomic(&(ctr))

static inline int
bfc_incr_i386_atomic(bfc_atomic_counter_t *cp)
{
    int32_t rc;
...
#define bfc_decr_atomic_counter(ctr) bfc_decr_i386_atomic(&(ctr))

static inline int
bfc_decr_i386_atomic(bfc_atomic_counter_t *cp)
{
    int32_t rc;
...
#define bfc_read_atomic_counter(ctr) (ctr)
