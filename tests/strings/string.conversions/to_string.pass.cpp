...
{
    {
        barefootc::basic_string<char> s = std::to_string(T(0));
        assert(s.size() == 1);
        assert(s[s.size()] == 0);
        assert(s == "0");
    }
    {
        barefootc::basic_string<char> s = std::to_string(T(12345));
        assert(s.size() == 5);
        assert(s[s.size()] == 0);
        assert(s == "12345");
    }
    ...
    {
        barefootc::basic_string<char> s = std::to_string(std::numeric_limits<T>::max());
    ...
    {
        barefootc::basic_string<char> s = std::to_string(std::numeric_limits<T>::min());
        std::istringstream is(s);
        T t(0);
        is >> t;
