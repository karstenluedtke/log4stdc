...
const struct bfc_string_class bfc_shared_iso8859_1_string_class = {
	...
	.traits		= (void *) &bfc_iso8859_1_traits_class,

	.substr		= bfc_shared_iso8859_1_substr,
				  ...
				  struct mempool *pool)
{
	...
	BFC_STRING_INIT_PROLOGUE(bfc_string_classptr_t,
			  ...
			  &bfc_shared_iso8859_1_string_class);
...
int
bfc_init_shared_iso8859_1_string(void *buf, size_t bufsize,
...
int
bfc_init_shared_iso8859_1_string_c_str(void *buf,size_t bufsize,const char *s)
{
	int n;
	l4sc_logger_ptr_t logger = l4sc_get_logger(BFC_STRING_LOGGER);
	...
	n = (*bfc_shared_iso8859_1_string_class.traits->szlen)(s);
	return bfc_init_shared_iso8859_1_string(buf, bufsize, s, n);
}

int
bfc_init_buffered_iso8859_1_string(void *buf, size_t bufsize,
				    const char *s, size_t n)
{
	l4sc_logger_ptr_t logger = l4sc_get_logger(BFC_STRING_LOGGER);
	...
	BFC_STRING_INIT_PROLOGUE(bfc_string_classptr_t,
			  bfc_strptr_t, obj, buf, bufsize, NULL,
			  &bfc_buffered_iso8859_1_string_class);
...
int
bfc_shared_iso8859_1_substr(bfc_cstrptr_t s, size_t pos, size_t n,
						void *buf, size_t bufsize)
{
	...
	return (bfc_init_shared_iso8859_1_string(buf, bufsize, data, sublen));
}
...
mutable_substr(bfc_cstrptr_t s, size_t pos, size_t n, void *buf, size_t bufsize)
{
	...
	bfc_init_shared_iso8859_1_string(buf, bufsize, data, sublen);
	return (-ENOSYS);
}

int
bfc_buffered_iso8859_1_substr(bfc_cstrptr_t s, size_t pos, size_t n,
				void *buf, size_t bufsize,
				void *databuf, size_t dbufsize)
{
	...
	rc = bfc_init_buffered_iso8859_1_string(buf, bufsize,
						 databuf, dbufsize);
	if (rc >= 0) {
