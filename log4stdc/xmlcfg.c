...
static int
init_xml_configurator(void *, size_t, bfc_mempool_t);
static size_t
...
{
    BFC_INIT_PROLOGUE(l4sc_configurator_class_ptr_t, l4sc_configurator_ptr_t,
                      configurator, buf, bufsize, pool,
...
on_start_tag(struct parsing_state *ps, const struct xml_tag *tag)
{
    ...
    if (ps->depth == 0) {
        for (i = 0; i < nattrs; i++) {
            if ((strncasecmp(attrs[i].name, "debug", 5) == 0) ||
                ...
                l4sc_set_internal_logging(attrs[i].value, attrs[i].vallen);
                break;
            }
        }
    }
    ...
    for (i = 0; i < nattrs; i++) {
        LOGDEBUG(("%s: attr #%d \"%s\" = \"%s\"", thisfunction, i,
    ...
    if (strncasecmp(name, "logger", 6) == 0) {
        ...
        for (i = 0; i < nattrs; i++) {
            l4sc_set_object_option((l4sc_objptr_t)ps->current_logger,
        ...
        appender = l4sc_get_appender(values.ref, values.reflen, NULL, 0);
        ...
        if (ps->current_layout) {
            l4sc_set_object_option((l4sc_objptr_t)ps->current_layout,
                                   values.name, values.namelen, values.value,
                                   values.vallen);
        ...
        if (ps->current_layout) {
            l4sc_set_object_option((l4sc_objptr_t)ps->current_layout, name,
                                   nmlen, values.value, values.vallen);
        } else if (ps->current_appender) {
            l4sc_set_object_option((l4sc_objptr_t)ps->current_appender, name,
                                   nmlen, values.value, values.vallen);
        } else if (ps->current_logger) {
            l4sc_set_object_option((l4sc_objptr_t)ps->current_logger, name,
                                   nmlen, values.value, values.vallen);
        }
    }
    return (BFC_SUCCESS);
...
configure_from_string(l4sc_configurator_ptr_t cfgtr, const char *s, size_t n)
{
    ...
    while ((rc = xml_find_next_tag(&tag, cp, limit)) >= 0) {
        iterations++;
        LOGDEBUG(("%s: iteration #%d, found %d, stack %p", thisfunction,
                  iterations, rc, &state));
                  ...
                  int maxattrs, int maxattrlen)
{
    ...
    for (; (pos < limit) && (n < maxattrs); pos++) {
        LOGDEBUG(("%s: pos %u/%u, have %u/%u", thisfunction, pos, limit, n,
                  maxattrs));
        ...
        c = s[pos];
        LOGDEBUG(("%s: pos %ld, char %c", thisfunction, (long)pos, (char)c));
        if (((c >= 'a') && (c <= 'z')) || ((c >= 'A') && (c <= 'Z'))) {
        ...
        if ((assign > start) && (quote > assign) && (endquote > quote)) {
            ...
            attrs[n].value[len] = '\0';
            attrs[n].vallen = len;
            LOGDEBUG(("%s(xmltag) #%d: %s=\"%s\"", thisfunction, n,
                      attrs[n].name, attrs[n].value));
            n++;
        }
    }
    return ((int)n);
    ...
    while (((amp = memchr(cp, '&', ep - cp)) != NULL) &&
        ...
        if (amp[1] == '#') {
            ...
            for (tp = html_entities; tp->ent; tp++) {
                if ((amp[1] == tp->ent[0]) && (strlen(tp->ent) == n) &&
